...
export const HW3Sprint2 = () => {
  ...
  const addNewFTRoute = (payload: { flightTableID: string; from: string; to: string }) => {
    const { flightTableID, from, to } = payload;
  ...
  const removeFTRoute = (payload: { flightTableID: string; routeID: string }) => {
    const { flightTableID, routeID } = payload;
    setFlightTables(
  ...
  const addNewFT = (payload: { from: string; to: string }) => {
  ...
  const updateFTDate = (payload: { flightTableID: string; newDate: string }) => {
    ...
    setFlightTables(
      flightTables.map(ft => (ft.flightTableID === flightTableID ? { ...ft, date: newDate } : ft))
    );
  };

  const updateFTRoutesFrom = (payload: {
    flightTableID: string;
    routeID: string;
    newFrom: string;
  }) => {
