...
type Props = {
  onClick: (payload: { from: string; to: string }) => void;
...
export const AddNewItem = ({ onClick, title }: Props) => {
  ...
  const addRouteHandler = () => {
    if (route.from.trim() !== "" && route.to.trim() !== "") {
      onClick({ from: route.from, to: route.to });
      setRoute({ from: "", to: "" });
    } else {
  ...
  return (
    <div className={styles.addNewItemWrapper}>
      <Input newTitle={route.from} setNewTitle={handleFromChange} placeholder="From" />
      <Input newTitle={route.to} setNewTitle={handleToChange} placeholder="To" />
